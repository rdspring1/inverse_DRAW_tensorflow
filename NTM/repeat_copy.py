...
MIN_REPEAT = 1
MAX_REPEAT = 4
OUTPUT_LEN = MAX_SEQUENCE_LEN * MAX_REPEAT 
INPUT_LEN = MAX_SEQUENCE_LEN + MAX_REPEAT + 1
...
def repeat_copy_generator(num_examples, size, min_seq_len, max_seq_len, min_repeat, max_repeat, SEED=None):
    np.random.seed(SEED)
    np_repeat_state = np.zeros([1, size])
    np_repeat_state[:, size-2] = 1

    np_end_state = np.zeros([1, size])
    np_end_state[:, size-1] = 1

    # n row - sequence termination
    # n-1 row - repeat_sequence
    idx = 0
    while idx < num_examples:
        ...
        max_input_len = max_seq_len + max_repeat + 1
        max_output_len = max_seq_len * max_repeat

        # Input random sequence 
        inputs_ = np.split(seq, seq_len, 0)

        for idx in range(num_repeats):
            inputs_.append(np_repeat_state)

        # Sequence End Marker
        inputs_.append(np_end_state)

        # Add Zero placeholders 
        for idx in range(len(inputs_), max_input_len, 1):
            inputs_.append(np.zeros([1, size]))

        output_ = np.zeros([max_output_len, size])
        for idx in range(num_repeats):
            start = idx * seq_len
            end = (idx+1) * seq_len
            output_[start:end, :] = seq
...
def main(argv=None):  # pylint: disable=unused-argument
    train_data = repeat_copy_generator(TRAIN_SIZE, SIZE, MIN_SEQUENCE_LEN, MAX_SEQUENCE_LEN, MIN_REPEAT, MAX_REPEAT, SEED)
